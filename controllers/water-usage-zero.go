...
func WaterUsageZero(c *gin.Context) {
	...
	result := configs.DB.Raw("SELECT a.kp_kode, COUNT(*) AS sm_jml, SUM(getUangAir(a.gol_kode, MONTH(CURDATE()), YEAR(CURDATE()), 0)) AS rek_uangair FROM tm_pelanggan a JOIN tm_stand_meter b ON b.pel_no = a.pel_no AND b.sm_thn = YEAR(CURDATE()) AND b.sm_bln = MONTH(CURDATE()) AND b.sm_kini = b.sm_lalu AND b.sm_sts = 1 GROUP BY a.kp_kode").Scan(&waters)
